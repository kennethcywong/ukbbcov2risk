...
dspath <- "/home/kenneth/out/UKBB/covid19/rerun_5Jan2021"
outpath <- "/home/kenneth/out/UKBB/covid19/rerun_tmp"
xgbmodel_path <- paste0(dspath, "/xgb_model/xgb_model_cohort_", cohort, "_fold_")

df.nlookup <- read.csv("meta/names_lookup_dot.csv", header = TRUE, stringsAsFactors=FALSE, quote="\"")

load(paste0(dspath, "/xgb_model/dataX_trainTestcomb_cohort", cohort, ".Rdata"))
...
save(mean.shap_val, file=paste0("mean.shap_val_lite_cohort", cohort,".Rdata"))
...
setwd(paste0(outpath, "/shapley"))
save(ind_subsamp, mean.int_val,mean.int_val_list_diffTraits, file= paste0("Interaction_Shap_lite", cohort, ".Rdata"))
...
setwd(paste0(outpath, "/shapley"))
pdf(paste0("Mean_shap_dependence_cohort", cohort, "_plot_lite_top6.pdf"),
